...
if !(has_state $state); then
     ...
     # Call Error Model
     format_status "Command:\nfastqutils merge $paramDir/modeled/$fileprefix.$subset.$condition.$readgroup.split.1.corfastq $paramDir/modeled/$fileprefix.$subset.$condition.$readgroup.split.2.cor.fastq"
     fastqutils merge $paramDir/modeled/$fileprefix.$subset.$condition.$readgroup.split.1.corfastq $paramDir/modeled/$fileprefix.$subset.$condition.$readgroup.split.2.cor.fastq
     # Update State on Exit
     status=$?
     put_state $status $state
